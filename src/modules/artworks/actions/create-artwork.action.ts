...
import { insertArtworkSchema, artworks } from "@/modules/artworks/schemas/artwork.schema";
...
export async function createArtworkAction(formData: FormData) {
    try {
        ...
        if (!validMagicBytes) {
             return { success: false, error: "Invalid file content. The file does not appear to be a genuine Image." };
        }
        ...
        if (!hash) {
             return { success: false, error: "File hash is required for integrity verification." };
        }

        // Upload to R2 (raw folder) with HASH as filename
        const uploadResult: UploadResult = await uploadToR2(imageFile, "raw", hash);
        ...
        const description = descriptionRaw ? (descriptionRaw as string) : undefined;
        ...
        const safeData = {
            ...
            protectionStatus: validatedData.protectionStatus as ProtectionStatusType,
        ...
        let result;
        try {
            result = await db.insert(artworks).values(safeData).returning({ insertedId: artworks.id });
        } catch (dbError) {
        ...
        if (newArtworkId) {
             ...
             console.log(`Triggering mock GPU at ${appUrl}/api/mock-gpu/process for ID ${newArtworkId}`);
             ...
             }).catch(err => console.error("Failed to trigger mock GPU:", err));
        }
        ...
        return { success: false, error: error.message || "Failed to create artwork" };
    }
}
