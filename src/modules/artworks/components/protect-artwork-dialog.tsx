...
export function ProtectArtworkDialog({
    ...
    const [selectedMethods, setSelectedMethods] = useState<ProtectionMethodType[]>([]);
    ...
    useEffect(() => {
        if (session?.user?.name && watermarkText === "DRIMIT SHIELD") {
            let sanitized = session.user.name.replace(/[^a-zA-Z0-9\sáéíóúÁÉÍÓÚñÑ.,!?-]/g, "");
    ...
    const hasConfigStep = selectedMethods.includes(ProtectionMethod.WATERMARK) || 
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[450px]">
                ...
                <div className="py-2">
                    ...
                    {step === 1 && (
                        <div className="space-y-4">
                            <h3 className="font-semibold text-sm text-foreground/80">Select Methods</h3>
                            <div className="grid gap-3">
                                {PROTECTION_OPTIONS.map((option) => {
                                    const Icon = option.icon;
                                    const isSelected = selectedMethods.includes(option.value as ProtectionMethodType);
                                                ...
                                                <div className="flex-1 space-y-1">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <p className="font-medium text-sm leading-none flex items-center gap-2">
                                                            {option.label}
                                                            <Badge variant="outline" className="text-[10px] h-4 px-1 py-0 border-muted-foreground/40 text-muted-foreground font-normal">
                                                                Coming Soon
                                                            </Badge>
                                                        </p>
                                                    </div>
                                    ...
                                    return (
                                        <div 
                                            ...
                                            onClick={() => toggleMethod(option.value as ProtectionMethodType)}
                                        >
                                            <Checkbox 
                                                checked={isSelected}
                                                onCheckedChange={() => toggleMethod(option.value as ProtectionMethodType)}
                                                className="mt-1"
                                            />
                                            <div className="flex-1 space-y-1">
                                                <div className="flex items-center gap-2">
                                                    <Icon className="h-4 w-4 text-foreground/70" />
                                                    <p className="font-medium text-sm leading-none">{option.label}</p>
                                                </div>
                                                <p className="text-xs text-muted-foreground">
                                                    {option.description}
                                                </p>
                    ...
                    {step === 2 && (
                        <div className="space-y-4">
                            <h3 className="font-semibold text-sm text-foreground/80">Pipeline Execution Order</h3>
                            <p className="text-xs text-muted-foreground">Reorder steps if necessary. The output of one step feeds into the next.</p>
                            
                            <div className="space-y-2">
                                {selectedMethods.map((method, index) => {
                                    const details = PROTECTION_OPTIONS.find(o => o.value === method);
                                    return (
                                        <div 
                                            key={method} 
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <Badge variant="outline" className="h-5 w-5 rounded-full flex items-center justify-center p-0 text-[10px] bg-background">
                                                    {index + 1}
                                                </Badge>
                                                <span className="text-sm font-medium leading-none">{details?.label || method}</span>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                ...
                                                <Button 
                                                    ...
                                                    size="icon" 
                                                    className="h-7 w-7"
                                                    disabled={index === selectedMethods.length - 1}
                        ...
                        <div className="space-y-6">
                            ...
                            {selectedMethods.includes(ProtectionMethod.MIST) && (
                                <div className="space-y-3">
                                    <Label className="text-xs font-semibold uppercase text-muted-foreground">AI Poisoning Strategy</Label>
                                    <div className="space-y-2">
                                        <Label htmlFor="model-select" className="text-sm">Select Model</Label>
                                        <Select value={mistModel} onValueChange={setMistModel}>
                                            <SelectTrigger id="model-select">
                                                <SelectValue placeholder="Select a model" />
                                            </SelectTrigger>
                                            <SelectContent>
                                                <SelectItem value="mist-v2">Mist V2 (High Performance)</SelectItem>
                                                <SelectItem value="glaze" disabled>Glaze (Coming Soon)</SelectItem>
                                                <SelectItem value="nightshade" disabled>Nightshade (Coming Soon)</SelectItem>
                                            </SelectContent>
                                        </Select>
                                        <p className="text-xs text-muted-foreground">
                                            Current active model: Mist V2 (Optimized for Style Transfer protection).
                                        </p>
                                    </div>
                                </div>
                            )}

                             {selectedMethods.includes(ProtectionMethod.MIST) && selectedMethods.includes(ProtectionMethod.WATERMARK) && (
                            ...
                            {selectedMethods.includes(ProtectionMethod.WATERMARK) && (
                                <div className="space-y-3">
                                    <Label className="text-xs font-semibold uppercase text-muted-foreground">Watermark Settings</Label>
                                    <div className="space-y-2">
                                        <Label htmlFor="watermark">Text Content</Label>
                                        <Input
                                            id="watermark"
                                            value={watermarkText}
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                            This text will be tiled diagonally across the protected image.
                                            Max 25 characters.
                                        </p>
                                    </div>
                                </div>
                        ...
                        <div className="space-y-6">
                             <div className="space-y-1">
                                ...
                                <p className="text-sm text-muted-foreground">
                                    This process runs in the background. It takes about <span className="font-semibold text-foreground">15 minutes</span>. 
                                    You can close this window safely.
                                </p>
                            </div>

                            <div className="space-y-2">
                                <h4 className="text-xs font-semibold uppercase text-muted-foreground tracking-wider">Final Pipeline</h4>
                                <div className="space-y-2">
                                     {selectedMethods.map((method, idx) => (
                                         <div key={method} className="flex items-center gap-2 text-sm">
                                             ...
                                             <span>
                                                 {PROTECTION_OPTIONS.find(o => o.value === method)?.label}
                                                 {method === ProtectionMethod.WATERMARK && (
                                                     <span className="text-muted-foreground ml-1">("{watermarkText}")</span>
                                                 )}
                                                 {method === ProtectionMethod.MIST && (
                                                     <span className="text-muted-foreground ml-1">({mistModel})</span>
                                                 )}
                                             </span>
                                             {idx < selectedMethods.length - 1 && (
                    ...
                    {step === 5 && (
                        <div className="flex flex-col items-center justify-center py-6 text-center space-y-4 animate-in fade-in zoom-in duration-300">
                             ...
                             <div className="space-y-2">
                                 <h3 className="text-lg font-semibold">Protection Started!</h3>
                                 <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                                     Your artwork is now being processed by our secure pipeline. You can safely close this window.
                                 </p>
                             </div>
                        </div>
                    )}
                ...
                <DialogFooter className={cn("flex flex-row items-center gap-2 sm:justify-end", step === 5 ? "justify-center sm:justify-center" : "justify-end")}>
                    {step > 1 && step < 5 && (
                     ...
                     {step < 4 ? (
                         <Button onClick={handleNext} disabled={selectedMethods.length === 0}>
