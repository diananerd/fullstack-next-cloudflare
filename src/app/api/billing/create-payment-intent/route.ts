...
export async function POST(req: Request) {
    try {
        ...
        try {
            ...
            console.log(
                "[API] Could not retrieve Cloudflare context, using process.env",
            );
        }

        if (!stripeKey) {
            console.error(
                "[API] STRIPE_SECRET_KEY is missing in environment variables",
            );
            ...
            console.log(
                `[API] STRIPE_SECRET_KEY loaded (length: ${keyLength})`,
            );
        }
        ...
        console.log(
            `[API] Creating payment intent for User=${session.user.id}, Amount=$${amount}`,
        );
        ...
        if (!customerId) {
            console.log(
                `[API] User ${session.user.id} has no Stripe Customer ID. Creating one...`,
            );
            try {
                ...
                console.log(
                    `[API] Created and saved new Stripe Customer ID: ${customerId}`,
                );
            } catch (e) {
        ...
        console.log(
            `[API] PaymentIntent created successfully. ID=${paymentIntent.id}`,
        );
