...
export async function POST(req: Request) {
    try {
        ...
        const { paymentIntentId } = (await req.json()) as {
            paymentIntentId: string;
        };

        if (!paymentIntentId) {
            return new NextResponse("Missing PaymentIntent ID", {
                status: 400,
            });
        }
        ...
        if (paymentIntent.metadata.userId !== session.user.id) {
            console.warn(
                `[Verification] User ${session.user.id} tried to verify payment ${paymentIntent.id} belonging to ${paymentIntent.metadata.userId}`,
            );
            return new NextResponse("Unauthorized Payment Access", {
                status: 403,
            });
        }
        ...
        if (creditsAmount > 0) {
            console.log(
                `[Verification] Verifying and Adding ${creditsAmount} credits for user ${session.user.id}`,
            );
