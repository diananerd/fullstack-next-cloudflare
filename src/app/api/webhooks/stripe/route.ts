...
export async function POST(req: Request) {
    ...
    try {
        ...
        console.warn(
            "[Webhook] Failed to get Cloudflare context, using process.env",
        );
    }
    ...
    if (!webhookSecret) {
        ...
        console.log(
            `[Webhook] STRIPE_WEBHOOK_SECRET present (length: ${webhookSecret.length})`,
        );
    }
    ...
    try {
        if (!signature || !webhookSecret) {
            ...
            console.error(
                "[Webhook] Missing stripe signature or webhook secret",
            );
        ...
        console.log(
            `[Webhook] Event Verified: ${event.type} (ID: ${event.id})`,
        );
    } catch (err: any) {
        console.error(
            `[Webhook] ⚠️  Signature verification failed.`,
            err.message,
        );
    ...
    if ((event.type as string) === "v2.core.event_destination.ping") {
        console.log(
            "[Webhook] 📡 Received Stripe V2 Ping. Connection verified.",
        );
    } else if (
        event.type === "payment_intent.succeeded" ||
        (event.type as string).endsWith(".payment_intent.succeeded")
    ) {
        ...
        if (event.object.startsWith("v2.")) {
            ...
            if (relatedObject?.id && relatedObject?.type === "payment_intent") {
                console.log(
                    `[Webhook] 🔄 Fetching full PaymentIntent from Thin Event: ${relatedObject.id}`,
                );
                ...
                console.warn(
                    "[Webhook] ⚠️ V2 Event received but could not resolve PaymentIntent ID.",
                );
                return new NextResponse("Unhandled V2 Event Structure", {
                    status: 400,
                });
            }
        } else {
        ...
        console.log(
            `[Webhook] Processing PaymentIntent ${paymentIntent.id} for User ${userId}, Credits: ${creditsAmount}`,
        );

        if (userId && creditsAmount > 0) {
            try {
                console.log(
                    `[Webhook] 💰 Adding ${creditsAmount} credits to user ${userId}...`,
                );
            ...
            console.warn(
                "[Webhook] ⚠️ PaymentIntent missing metadata (userId or credits). Skipping credit addition.",
            );
        }
    } else {
