...
export async function PUT(req: NextRequest, props: { params: Promise<{ id: string }> }) {
    try {
        ...
        const bucket = (env as any).R2_BUCKET || (env as any).BUCKET || (env as any).drimit_shield_bucket;
        ...
        const userConfig = meta.config && typeof meta.config === 'object' ? meta.config : {};
        ...
        const pipelineConfig = [{
            ...
            config: {
                ...
                watermark_text: userConfig.watermark_text || "DRIMIT SHIELD",
                ...userConfig
            }
        }];

        await PipelineService.startPipeline(artwork.id, artwork.userId, pipelineConfig);
