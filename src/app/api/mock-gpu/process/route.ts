...
export async function POST(req: NextRequest) {
    ...
    try {
        const body = (await req.json()) as { artworkId: number; fileUrl: string };
        ...
        console.log(`[MockGPU] Starting protection for ${artworkId} (${fileUrl})`);
        ...
        console.log(`[MockGPU] Uploaded protected version to ${uploadResult.key}`);
        ...
        if (currentArtwork?.protectionStatus === ProtectionStatus.CANCELED) {
             console.log(`[MockGPU] Job for ${artworkId} was canceled. Cleaning up.`);
             if (uploadResult.key) await deleteFromR2(uploadResult.key);
             return NextResponse.json({ success: false, message: "Job canceled" });
        }
        ...
        return NextResponse.json({ success: true, message: "Protection applied (mock)" });
