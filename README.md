...
graph TD
    ...
    subgraph "AI Engine (Modal)"
        Pipeline -->|4. Dispatch via HTTP| ModalAPI[Modal Protection Service]
...
- **Schema Key Models**:
  ...
  - `artwork_jobs`: The granular unit of work. Tracks individual steps and their connection to external Modal job IDs.
...
- **Core Logic**: Located in `modal/protection/main.py` and `modal/simulation/main.py`.
- **Functionality**:
  - **Identity Shield**: Adversarial attacks against Face Recognition (ArcFace, InsightFace).
  - **Style Poison**: Adversarial attacks against Vision Transformers (CLIP, OpenCLIP).
  - **Edit Immunity**: Latent disruption against Generative models (Stable Diffusion, Flux).
  - **Provenance**: Invisible Watermarking (DWT/DCT).
- **State Management**: Uses `modal.Dict` (`shield-job-states`) to persist job statuses.
...
- **Modal**:
  - `modal deploy modal/protection/main.py` (Deploys the Python backend).
  - `modal deploy modal/simulation/main.py` (Deploys the Validation Engine).
