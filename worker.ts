...
export default {
    ...
    async scheduled(curr, env, ctx) {
         ...
         console.log(`[Worker] Triggering scheduled sync at ${new Date(curr.scheduledTime).toISOString()} (Cron: ${curr.cron})`);
    ...
    async queue(batch, env, ctx) {
        ...
        console.log("[Worker] Queue event received (should not happen)", batch.queue);
    }
};
